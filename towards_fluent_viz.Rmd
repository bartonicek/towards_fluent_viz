...
As was briefly mentioned in the section above, the production of visualizations may involve issues involving order and hierarchy. In static visualizations, all computation is done only once, before the plot is rendered, and so these issues are less important. However, when producing interactive visualizations, we want our visualizations to respond to user's input, which means that some computations may have to be run rapidly and repeatedly, often many times within a single second. If the computation takes a long time to complete, it may result in a sluggish or unresponsive visualization. As a result, it is imperative to organize the process in such a way that the computation is as fast as possible and, in turn, only as little work as is necessary is done. 
