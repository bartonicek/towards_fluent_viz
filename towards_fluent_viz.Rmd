...
Interactive data visualization is becoming increasingly popular. Interactive figures frequently appear in online news articles, business dashboards, and scientific publications and blogs.      

Different interactive features can involve vastly different levels of computational complexity. Some types of interaction can be implemented by manipulating the graphical attributes of the visualization only, without referring to the original data. For example, to implement panning or zooming, all we need to do is compute the axis limits from the data once and then merely update the four scalar values (lower and upper x-axis and y-axis limits) whenever the user performs the requisite actions. However, there are also more complex types of interaction. For example, if we try to implement an interactive histogram in which the user can change the binwidth and anchor, we need a way to recompute the number of cases in each bin after either of the two parameters is updated. Likewise, to implement linked highlighting/brushing, whereby the user can highlight some cases in the data across multiple plots by either clicking or click-and-drag-selecting the corresponding geometric objects, we need to keep track of which cases belong to which object.

All this to say, there is a lack of a general theory of interactive data visualizations. There have been open calls for an interactive data visualization pipeline or "plumbing" [@wickham2009] and some attempts to address these calls have been made [@xie2014]. However, there is still a 

## Brief history of static data visualization

Data visualization has a long and intricate history [see e.g. @dix1998;@chen2008;@friendly2021;@young2011]. Briefly, early examples of data visualization existed well before the 20th century, such as in the works of William Playfair [see e.g. @costigan1990]. However, for a long time, it was considered at best an auxiliary field to mathematical statistics. This began to change at the end of 1950's, when a series of developments took place that lead to a great increase in the prominence of data visualization. Firstly, at the theoretical level, the work of Tukey [-@tukey1962;-@tukey1977] and Bertin [-@bertin1967] established data visualization as an independent discipline in its own right. Secondly, at the applied level, the development of personal computers [see e.g. @abbate1999] and high-level programming languages, most notably FORTRAN in 1954 [@backus1978], distributed the ability to produce publication-quality graphics to the wider public. Combined, these developments lead to a surge in the popularity of data visualization and dissemination of figures. 

Final development in the field of static data visualization that is important to mention was the development of the Grammar of Graphics, introduced by Leland Wilkinson [-@wilkinson2012]. Prior to Wilkinson's work, data visualization systems tended to come in two forms: low-level systems, in which the users had to create visualizations from scratch by working with graphical primitives, and high-level systems, in which the users could select from a limited range of ready-made classes of plots (such as scatterplot, barplot, etc..). Wilkinson, building upon the work of Bertin and Tukey, developed theory for a mid-level visualization system - Grammar of Graphics - which allows the users to specify a broad range of statistical graphics declaratively, by combining composable pieces which specify a graphic, including aesthetics, scales, coordinates, and geometric objects [@wilkinson2012]. Grammar of Graphics has been successfully implemented in several software packages, most notably the popular *ggplot2* R package [@wickham2010], and the proprietary software *Tableau* [@tableau2023].

## Early interactive data visualization

After static data visualization entered the computer age, it would not take long for interactive data visualization systems to appear. Early systems appeared in the 1960's and 1970's, and tended to be specialized for one specific task. For example, [@fowlkes1969] used interactive visualization to show how probability densities reacted to change of parameters and transformations, and [@kruskal1964] used interactive visualization to showcase his multi-dimensional scaling algorithm (a way of embedding objects within a common space based on pairwise distance measurements). The first "general-purpose" system was *PRIM-9* [@fisherkeller1974], which allowed for exploration of high-dimensional data in scatterplots using projection, rotation, subsetting and masking. Later systems grew on to become even more general and ambitious. For example, *MacSpin* [@donoho1988], *Lisp-Stat* and *XLisp-Stat* [@tierney1989; @tierney2004], and *XGobi* [@swayne1998] provided rich features such as interactive scaling, rotation, linked selection (or "brushing"), and interactive plotting of smooth fits in scatterplots, as well as interactive parallel coordinate plots and grand tours.

Following the turn of the 21st century, interactive data visualization systems further grew in scope and flexibility and began to be integrated into general-purpose statistical computing software. The successor system to XGobi, *GGobi* [@swayne2003] was made to be directly embeddable in R. Java-based *Mondrian* [@theus2002] allowed for sophisticated linked interaction between many different types of plots including scatteplots, histograms, barplots, scatterplot, mosaic plots, parallel coordinates plots, and maps. Similarly, *iPlots* [@urbanek2003] implemented a general framework for interactive plotting that was not only embedded in R but could be directly programmatically manipulated, and was later further expanded and made performant for large data volumes in *iPlots eXtreme* [@urbanek2011]. Finally, *cranvas* [@xie2014] also implemented a similar suite of interactive features.

## Web-native interactivity

While the developments outlined in the previous section took place, the rise of and proliferation of Web technologies in the mid 1990's provided another popular avenue for interactive graphics. Specifically, the appearance of JavaScript in 1995 as a high-level general-purpose programming language for the Web [for a description of the history, see e.g. @wirfs-brock2020] created an extremely versatile platform for highly-reactive and portable applications. JavaScript was created with the explicit purpose of making the Web interactive, and the fast dissemination of standardized web-browsers meant that online interactive applications could be accessed by anyone, from anywhere. Interactive data visualization became just one of many technologies highly sought after within the fledgling Web ecosystem. 

The very early systems such as *Prefuse* in 2005 [@heer2005] and *Flare* in 2008 [@flare2020] relied on plugins (Java and Adobe Flash Player, respectively). However, in the early 2010's, several true Web-native interactive data visualization systems emerged. The most prominent among these is *D3*.js [@bostock2011]. D3 is a broad and general framework for manipulating data and HTML documents and creating visualizations. It is in wide use still to this day and has spawned a number of specialized, higher-level data visualization libraries that abstract away some of the details and provide an interface to commonly used statistical plots, such as the also very prominent *plotly*.js [@plotly2022]. Importantly, in the D3 model, D3's engine only renders graphics: interactivity is left the user. If a user wants to create a visualization that's interactive, they have to write JavaScript functions that take care of updating the underlying parameters. A radically different approach altogether was taken by *Vega* [@satyanarayan2015]. In Vega, all aspects of a visualization including interactivity are specified declaratively, as a Plain Old JavaScript Object (POJO). As such, Vega carries a large number of reactive primitives. Similar to D3 and plotly, Vega also spawned its own higher-level libraries such as *Vega-lite* [@satyanarayan2016], and *Altair* [@vanderplas2018]. One final popular interactive data visualization framework in JavaScript is *Highcharts* [@highcharts2023]. Similar to Vega-lite, Highcharts is a high-level declarative framework in which plots are created based on a POJO specification object. Finally, besides the general interactive data visualization frameworks, there are also many more specialized libraries focusing on e.g. rendering interactive scatterplots at large data volumes [see e.g. @lekschas2023].   
...
In some treatments of data visualization, the summarizing step ($\alpha: D \to S$) is de-emphasized, in one of two ways. In the first case, the data is assumed to arrive pre-summarized, i.e. $S = D$. An example of this would be drawing a barplot from a pre-summarized table of counts. Under this framing, data visualization is just about encoding the data into geometric objects. This may feel very natural when considering plots which show a one-to-one mapping (bijection) between the data and the geometric objects, such as the scatterplot or the lineplot/time-series chart. And indeed, in these plots, the summarizing function $\alpha$ is identity, and as such *can* be ignored. In the second case, the computation of summaries is considered, however, it is absorbed into the translation step, such that $\alpha = \beta$. As an example of this, we may think of a histogram as directly translating its underlying variable into bin coordinates. Thus, under this framing, data visualization is also about encoding the data into graphics, although in this case there is "more going on" in the encoding step. 

Neither of these two approaches is *wrong*. The data visualization process, by its very nature, consists of *composition* (that is the essence of (\ref{eq:recipe1}) and (\ref{eq:recipe2})). That is, given two functions $\alpha$ and $\beta$ , we can define a new composite function $\rho = \beta \circ \alpha$ (read "beta after alpha"), that takes in the input of $\alpha$ and spits out the output of $\beta$, i.e. $\rho(d) = \beta(\alpha(d))$. And indeed, this is what we want and expect from our data visualization systems - it would quickly get tiring if instead of "visualization" we had to talk about "summarizing-then-translating-then-rendering".  

However, composing things and abstracting them out into larger blocks or chunks can also bring some disadvantages. Specifically, in the case of the two approaches outlined above, there is a tight coupling between summaries and graphical coordinates. For example, under the first framing, which assumes that data arrives pre-summarized, it may be awkward or unnecessarily laborious to make all but the most simple types of plots - most users of data visualization systems would probably not enjoy having to go through several data-wrangling steps each time they wanted to draw a boxplot. Conversely, under the second framing, which assumes that summaries are just part of translating the raw data into graphics, we need a special "class" of a plot for every different summary, e.g. a barplot of counts, a barplot of sums, a barplot of means, etc...

We can give a much richer and general account of visualizations by treating both the summarizing and the translating steps as first class citizens. Under this framework, some plots may show the same summaries but encode them through different graphical attributes and geometric objects, and, conversely, other plots may compute different summaries but display them via the same means. As an example of the former, see the examples of histogram and spineplot in Figure \@ref(fig:histospine). Histograms and spineplots both use the same summaries, specifically binned counts of some underlying continuous variable $x$. However, the histogram encodes the bin breaks into rectangle boundaries along the x-axis, and the counts as the top rectangle boundary along the y-axis, whereas the spineplot encodes the counts into both x- and y-axis rectangle boundaries, with both the x- and the y- dimensions stacked and the y-dimension additionally scaled to 1, and the bin breaks are encoded as x-axis labels. As an example of the latter, scatterplot and bubbleplot both use points/circles to display their underlying summaries, however, whereas the summary for scatterplot is identity (or at least some bijective function), the summary for bubbleplot involves binning along x- and y- axis.  
...
There is an even deeper issue when it comes to interactivity and statistical summaries of the data. Specifically, not every statistical summary "works" equally well - instead, some summaries produce better or more coherent interactive graphics than others. Let's first illustrate the problem with a case study.
...
First of all, we need to think about how the information from the data will be distributed among the objects. Sometimes, we may want all information in the data displayed as a single geometric object. This is a perfectly good way of displaying information about the entire dataset. Examples of plots with a single geometric object include a single-time-series plot (line), a density plot (density polygon), a or radar chart (pentagon/other polygon).  

However, often, we may want to compare different parts of the data via different objects. And this is where we have to start to think more deeply about how the information should be allocated. For example, it would not be very useful to draw multiple identical geometric objects representing the same information from the data - if all of the objects looked the same, we could learn the same information from any one of them and so it is redundant to have more than one. Instead, we want each object to represent different aspects of the data and so we need to think about slicing the data up along some axis or axes.  
...
The reduce operation can be summarized with the following R pseudocode:

```{r, eval = FALSE}

factor_indices <- ... # E.g. c(1, 1, 2, 1, 3 ...)
parts <- list()

reduce <- ...
reduce_initial <- ... # E.g. function() list(count = 0, sum = 0)

for (j in 1:k) parts[[j]] <- reduce_initial()

for (i in 1:n) {
  parts[[factor_indices[i]]] <- reduce(parts[[factor_indices[i]]], data[i, ])
}
...
#| echo: false
#| fig-height: 3
#| fig-cap: "Diagram of the reduce operation: we iterate through $n$ datapoints, updating the appropriate collection of summaries (one of $k$, ) at each step."
...
grid.newpage()
grid.text("k reduced collections", 0.5, 0.9)
...
Once we have computed the $k$ collections of summaries, we may also want to iterate over them. There are two reasons. First, we may want to *map* (translate) each collection somehow. For example, we may want to normalize the summaries (possibly by parent-part values), translate them to graphical attributes/coordinates, etc... This operation takes each collection of summaries and returns new, mapped collection.

```{r}
#| echo: false
#| fig-height: 3
#| fig-cap: "Diagram of the map operation: we iterate through the $k$ collections of summaries, translating each collection at each step."
...
We may also want to *stack* some summaries/attributes on top of each other, as in the case of stacked barplot bars. This is somewhat more challenging, since we need to stack the appropriate summaries together and in appropriate order. For example, if we have 6 collections of summaries, representing the product of $\textbf{gender} = (male, female)$ and $\textbf{group} = (A, B, C)$ factors - $\{ (male; A), (male; B), (male; C), (female; A), (female; b), (female; C) \}$ - we need to stack collections which belong to $male$ together and collections which belong to $female$ together, respectively, and in the order of the groups: $A, B, C$. To do this, the collections should: a) be ordered appropriately, and b) each have an index or reference to the parent collection. We can achieve this as follows. First, assuming that the two factors which constitute the product factor have $k_1$ and $k_2$ levels, indexed by integers $i_1$ and $i_2$, respectively, we can define the product factor index as: $i_3 = k_2 \times i_1 + i_2$. This will guarantee that the levels have first-factor-major order (i.e. $\{ (male; A), (male; B), \ldots \}$, as outlined above). Second, the index of reference to the parent collection should be kept e.g. as part of the factor data. With these in place, we can stack as follows:       

```{r}
#| echo: false
#| fig-height: 3
#| fig-cap: "Diagram of stacking: we iterate through the $k$ collections of summaries, each time first updating the collection of stacked values (belonging to the appropriate parent part) by the part values and then copying the stacked values into the part."
...
The map and stack operation can be summarized with the following pseudocode:

```{r, eval=FALSE}

parts <- ... # E.g. as was computed above
parent_parts <- ...
parent_indices <- ...
stacked <- list()

map <- ...   # function(part) { ... }
stack <- ... # function(parent, part) { ... }
stack_initial <- ... 

for (jj in 1:k_parent) stacked[[jj]] <- stack_initial()

for (j in 1:k) {
  # We may need to map with parent values, e.g. to normalize
  parts[[j]]$parent <- parent_parts[[parent_indices[j]]] 
  mapped_part <- map(parts[[j]])
  
  stacked[[parent_indices[j]]] <- stack(stacked[[parent_indices[j]]], mapped_part)
  parts[[j]] <- stacked[[parent_indices[j]]]
}
