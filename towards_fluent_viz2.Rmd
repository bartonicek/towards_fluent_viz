...
Interactive data visualization (IDV) has seen a rapid growth in popularity over the last few decades. From the humble roots of the early specialized systems, such as those of @fowlkes1969 and @kruskal1964, there has been a steady progress towards more and more general and feature-rich frameworks. Of these, the first general-purpose system was *PRIM-9* [@fisherkeller1974], which allowed for exploration of high-dimensional data in scatterplots using projection, rotation, subsetting and masking. Later systems, such as *MacSpin* [@donoho1988], *Lisp-Stat* and *XLisp-Stat* [@tierney1989; @tierney2004], and *XGobi* [@swayne1998] provided rich features such as interactive scaling, rotation, linked selection (or "brushing"), and interactive plotting of smooth fits in scatterplots, as well as interactive parallel coordinate plots and grand tours. They were later followed by other systems such as *Mondrian* [@theus2002], *GGobi* [@swayne2003], *iPlots* [@urbanek2003], and *cranvas* [@xie2014]. Alongside these later developments, which have largely come from the field of statistics, the rise of Web technologies and interactive web apps has spawned its own family of IDV systems. Among these, the earlier systems such as *Prefuse* [@heer2005] and *Flare* [@flare2020] relied on external plugins (Java and Adobe Flash Player, respectively), while later systems became truly Web-native by embracing JavaScript. Among them, *D3*.js [@bostock2011] has grown to great popularity, alongside its high-level interface *plotly*.js,  [@plotly2022], and so have other frameworks such as *Vega* [@satyanarayan2015], *Vega-lite* [@satyanarayan2016], *Altair* [@vanderPlas2018], and *Highcharts* [@highcharts2023].  

Interactive figures now frequently appear in online news articles, business dashboards, and scientific publications and blogs. Yet, despite the wide proliferation of IDV systems and various taxonomies of interactive features [see e.g. @yi2007], there still looms a large unresolved problem related to the production of IDVs. That is, interactive data visualizations, like all visualizations, require us to process data into statistical summaries that can then be translated into graphical attributes and coordinates and drawn. However, in contrast to static visualizations, where all data processing needs to occur only once, IDVs need to respond to user input reactively, with relevant quantities often having to be recomputed many times a second [@wickham2009]. Further, different types of interactions come with different levels of computational complexity [see e.g. @leman2013; @pike2009]. 

Thus, the way we implement the data processing step matters a lot. Some interactions can be implemented by manipulating graphical attributes of the visualization only. For example, to implement panning or zooming, all we need to do is compute the axis limits once. After that, we can "forget" the original data and merely update the four scalar values (the lower and upper x-axis and y-axis limits), whenever the user performs the requisite actions. However, this is not the case for other, more complex types of interactions. For example, to be able to change binwidth and anchor in an interactive histogram, we need to recompute the number of cases in each bin whenever either of the two parameters is updated. Similarly, to implement linked highlighting/brushing across multiple plots, such that the user can highlight some cases in the data by selecting the corresponding geometric objects in one plot, we need to keep track of which cases belong to which object and then recompute the relevant statistics on the fly whenever necessary. 

There have been calls for a development of a theory of an interactive data visualization pipeline or "plumbing" [@wickham2009], that would allow us to reason about the data processing step in the production of IDVs in a similar way that, for example, the *Grammar of Graphics* [@wilkinson2012] has allowed us to reason about the production of static visualizations. However, these calls have, despite some attempts [see e.g. @xie2014; @crossfilter2023], been left largely unaddressed.  
 
# The illusion of objects

The key towards an IDV pipeline may lie in a subtle yet profound question that inevitably comes up in the production and use of interactive data visualizations: *when we interact with a plot, what exactly are we interacting with?* On its face, it may seem trivial. A person clicking a bar in an interactive barplot may be convinced that they are interacting with the coloured rectangle on the screen, since, by design, that is the salient "thing" they see change in front of them. And in some way, this is true - by interacting with the bar, we can affect its graphical attributes: we can change its colour, we can squeeze it/stretch it, and so on. Yet, in another, deeper way, this perception of interacting with a plain geometric object is just an illusion. How so? 

The illusion lies in the fact that the bar is not just the geometric object - the coloured rectangle - it is represented by. Instead, the rectangle is only ever meaningful as a "bar" within the context of the plot. We can see this easily - if we were to take the coloured rectangle outside of the plot, we would lose some crucial information that the rest of the plot provides.  
...
Thus, objects in plots are imbued with some additional information or structure, beyond their simple geometries. That should not seem surprising or controversial to people familiar with data visualization. Indeed, geometric objects or `geoms` are considered just one component of a plot in the *Grammar of Graphics* [@wilkinson2012] and its popular implementation *ggplot2* [@wickham2009]. 

However, what is this "structure" behind geometric objects? We may be able to muster a few ideas. First of all, we know that the geometric objects in plots are supposed to represent some underlying data. That much is clear - if the objects in a graphic do not represent any external data but are instead drawn according to some arbitrary rules, we cannot really, in good conscience, call the resulting graphic a "plot". Thus, every geometric object or "mark" in a plot (outside of auxiliary marks such as axis ticks, gridlines, etc...) has to directly represent one or more cases of the data.

But data is only a part of the story. When drawing plots, we rarely represent the data directly, in its raw form. Instead, we often summarize, aggregate, or transform [see @wilkinson2012, Chapter 7]. We do this by applying mathematical functions such as count, sum, mean, log, or the quantile function. In *ggplot2*, these are the less well-known but no less important counterparts to `geoms`: statistics or `stats` [@wickham2009]. It is the output of these summary functions that we represent by geometric objects.

So, when interacting with a bar in an interactive barplot, we do not just interact with a rectangle. Instead, we interact with a mathematical function, or, in fact, several of them. To the end-user of visualizations this may not seem particularly relevant, but it is, for one very important reason: functions have properties, and these properties may impose limits on what kinds of visualizations and interactions we can meaningfully compose. This is the core argument of the present text. Before diving deeper, however, let's first define some key terms and draw a rough sketch of the data visualization process as a whole. 

# Rough sketch of the data visualization process

To create a data visualization, be it static or interactive, we need several ingredients: data, summaries, scales/coordinate systems, and geometric objects. These should be familiar to most users of interactive data visualization systems. However, it may still be useful to lay them out in order, and examine the specific features of each one of them.    

First of all, every data visualization needs to be built on top of some underlying data. We can represent this as a set of some arbitrary units of information (or "data points") $D$. Data in the wild usually comes with more structure than that - for example, we often encounter data stored in a tabular [or even "tidy", @wickham2014] format, stratified by rows and columns. In that case, we could represent $D$ as the set of rows $R$, the set of columns $C$, or the set of cell values $R \times C$ (where $\times$ indicates the cartesian product). However, for the present purpose, we do not have to assume any special structure for the data, and can instead just speak of $n$ data points $d \in D$.  

Second, at some point during the visualization process, we need to transform the data points $D$ into a set of collections of summaries $S$ via some function $\alpha$. This summarizing function can come many different forms. It may be the case that $\alpha$ is one-to-one (bijection), in which case there is one collection of summaries for every unit of data (and vice versa). This is the case, for example, in the typical scatterplot, in which $\alpha$ is just the identity function (every row of data gets assigned one point). However, more often, $\alpha$ is many-to-one, which means that multiple units of data may be merged or combined into a single summary. Examples of this include the typical barplot, histogram, density plot, violin plot, mosaic plot, and many others. When $\alpha$ is many-to-one, it will often reduce the cardinality of the data, such that $k = \lvert S \lvert  \leq \lvert D \lvert = n$ (e.g. in a typical barplot, there are fewer bars than there are rows of data, unless there is a unique level of the categorical variable in each row). 

To turn the $n$ units of data into $k$ collections of summaries, we need to somehow stratify the data on one or more variables. These variables may either come from the data directly (i.e. the variables used are "factors", as in the case of a barplot or a treemap) or may themselves be a summary of the data (as in the case of histogram bins). Importantly also, each collection of summaries $s \in S$ may (and usually will) hold multiple values, produced by a different constituent function each. For example, the collection of summaries $s$ for a single boxplot "box" will consist of the median, the first and third quartile, the minimum and maximum, and the outlier values of some variable, all for a given level of some stratifying variable (which itself will also be an element of $s$). Finally, the output of these constituent functions may also depend on some external parameters, which may be either directly supplied by the user or heuristically inferred from the data by the visualization system. Examples of such external parameters include the anchor and binwidth in a histogram, the density bandwidth in a density plot, or the list of knots for a smooth fit.
...
The above should be fairly non-controversial description of how data visualizations are produced, and applies equally well to static as well as interactive visualizations. There are a few caveats, however. Firstly, while not always the case, *order* can be important. For example, while the order of the bars in a barplot along the x-axis is usually arbitrary (and typically defaults to alphabetical order), lines in a lineplot need to be drawn by connecting points in a specific order, otherwise we end up with completely different-looking objects (lines). Overall, this means that either $S$, $G$, or their components, may need to be ordered. Secondly, *hierarchy* can also be important and the collections may form tree/graph-like structure. This is particularly relevant when stacking. For example, when drawing a stacked barplot, we need to stack the graphical coordinates representing the sub-bars on top of each other, such that each sub-bar is stacked within the appropriate parent-bar and in the correct order (e.g. such that sub-bars belonging to group 2 always go on top group 1 sub-bars). If it is the case that order matters for either $\alpha$ or $\beta$, then we cannot call them true functions either, since each of the elements in their domain needs to be aware of the other elements. That is, when stacking bars in a stacked barplot, we need to "remember" what bars we have stacked before. 

# The problem of statistical summaries {#problem}

<!-- These operations work well with statistics like counts, means, propor- -->
<!-- tions, and standard deviations. Simple aggregations over subclasses can be -->
<!-- computed by operating on sums, sums of squares, and other terms that are -->
<!-- combined in linear functions to produce basic summary statistics. They do not -->
<!-- work properly with statistics like the median, mode, and percentiles because -->
<!-- the aggregate of these statistics is not the statistic of their aggregates. The me- -->
<!-- dian of medians is not the median of the aggregate, for example. A telling in- -->
<!-- dicator of this fact is the lack of medians and modes from the menus of -->
<!-- MOLAP front-ends. Grammar of Graphics p. 52 -->
...
>
> Grammar of Graphics, @wilkinson2012, p. 15 and p. 112
...
>
> Response to a StackOverflow post about linked brushing by user Leo [@jakevdp2019]

Returning to the example of the "illusory rectangle" in a barplot, we can discuss the process of interacting with the summarizing function $\alpha$. In a typical barplot, $\alpha$ consists of the simple operation of counting how many times each of the $k$ unique levels of the categorical variable appeared in the data. But $\alpha$ does not *have* to consist of counting. We could substitute a different statistical summary for $\alpha$, e.g. taking the sum or mean of the values of some other (continuous) variable in the data. Indeed, this is what we can do by specifying the `stat` argument in *ggplot2* plots.  

However, herein lies the trap - just because we can substitute a *different* function for $\alpha$, this does not mean *every* function will work. If our goal is to produce coherent visualizations with specific features, only some summarizing functions will do. This is especially true for interactive visualizations, since interactive features impose additional constraints on the visualization. We will illustrate the problem with a case study of implementing linked brushing.

## Linked brushing

Linked brushing or highlighting is one of the most popular and useful interactive features used in IDVs [see e.g. @buja1996; @wilhelm2003]. It allows the user to select cases of the data corresponding to an object or objects in one plot, typically by either clicking to select one object or clicking-and-dragging to select multiple objects within a region. The selected cases are then highlighted across all other "linked" plots. The usefulness of linked brushing comes from the fact that it allows the user to rapidly "drill-down" [@dix1998; @theus2002] and compare different subsets of the data.

When all plots contain objects with a 1-to-1 (bijective) mapping of the data only (e.g. all plots are scatterplots), linked brushing is simple, since we can just manipulate the colour attribute across the $n$ points. Things become more difficult with 1-to-$k$ many-to-one mappings, however. For example, let's imagine we have a linked barplot and a scatterplot. Then, we need to keep track of how which cases each bar in the barplot represents, so that whenever we select that bar we know which points to highlight. But more importantly, in the reverse scenario, when we select points in a scatterplot, how should the barplot respond? We could simply highlight every bar which includes any of the selected case. However, this kind of mapping does not preserve proportionality - a single selected point in the scatterplot will highlight an entire bar. Further, rather than the question of *whether any* cases in a category have been selected, a more interesting question may be *what proportion*. This question is also a strict upgrade in terms of information density, since if non-zero proportion of cases have been selected, we know that *some* have. Thus we may be interested in somehow comparing the summary (count) of the subgroups induced by the brushing (selected vs. non-selected cases).

## Stacking vs dodging

There are many different ways of graphically displaying subgroups. One technique known as "dodging" or "grouping" [as in "grouped barplot", @wilke2019] consists of drawing the objects representing the subgroups side-by-side. For example, in our interactive barplot, we could draw two bars representing selected and non-selected cases side-by-side, for each category. Another option is "stacking", that is, drawing the subgroup bars on top of each other. Typically, this means adding the upper y-axis coordinate of the previous subgroup to the upper and lower y-axis coordinates of the next (although more on this later, see Section TODO).   

While dodging is often recommended over stacking when it comes to static visualizations, since it has better visual perception properties [@cleveland1984; for a short elaboration in the form of a blogpost, see @kosara2016], stacking seems to be the more popular technique in linked interactive visualizations. Why might this be? In static visualizations, the variable that induces the subgroups is fixed, and typically has all or most levels nested within most of the levels of the (supra-)category. In contrast, in linked IDVs, the variable that represents the selection changes with user interaction. As a result, the subgroups change rapidly and can become empty (i.e. either none or all cases are selected, within a single category). With dodging, this can lead to the bars "flickering in and out of existence", and the whole plot may end up looking "ragged", whenever some selection groups are empty. Further, if bidirectional linking is desired, in a dodged barplot it may not be obvious what the objects that can be selected are - will selecting one subbar select the whole category or just the intersection of that category and the selection? 

In contrast, with stacking, the height of the whole stacked bar representing one category does not change as a result of brushing - only the subbars do. As such, the visual outline of the plot remains constant throughout time. Further, when it comes to selection, it seems more intuitive that a stacked bar represents one whole objects, compared to two side-by-side bars. Thus, stacking is results in more visually pleasing and intuitive figures which tax the visual system less.

Thus, stacking seems to be a good choice when it comes to implementing linked brushing. However, if we stack, we have to be very mindful of *what* we stack. In order to be meaningfully stackable, the summaries we represent in a plot need to have specific properties. This is the topic of the next section.

## Case study: linked brushing of counts and means

Let's imagine we have three linked plots: a scatterplot, a barplot of counts within some categories, and a barplot summarizing the mean of some other variable within the same categories. To a naive user of IDVs, it might seem that both types of barplot would behave equally well under linked brushing and stacking. However, let's examine the situation in more detail.  

```{r empty, echo=FALSE }
#| fig.height: 3
#| fig.cap: "The problem of representing empty selection. a) An illustration of selection by linked brushing. b) In the barplot of counts, the count within an empty selection is zero and so an absence of a bar accurately represents the count of zero. c) In the barplot of means, the mean of an empty selection is not defined. Absence of a bar could indicate that either no cases are selected or that some cases are selected and their mean is equal to the lower y-axis limit (0, in this case."
...
First, as is shown in Figure \@ref(fig:empty), how do we represent empty selections? In the case of counts, we have a meaningful default value - zero - as in "the number of cases in an empty selection is zero". In other words, the absence of a bar in a barplot unambiguously indicates that the count within the corresponding category is zero.
...
#| echo: FALSE
#| fig.height: 3
#| fig.cap: "The relationship between subset vs. the whole. a) An illustration of selection by linked brushing. b) In the barplot of counts, the count within a selection (red) is always less than or equal to the count within the whole and the outline of the bars does not change. c) In the barplot of means, the mean of a selection can be greater than the mean of the whole."
...
### Subset vs. the whole {#lessthan}

Second, as shown in Figure \@ref(fig:bigger), how does the summary on the selection subset relate to the summary on the whole? In the case of the barplot of counts, the height of the sub-bar is always less than or equal ($\leq$) to the height of the whole stacked bar (because so is the count; see Figure \ref(fig:bigger)). Thus, the outline of the whole plot will not change with brushing. Further we can choose to draw the stacked bar in two different ways: either by "layering" bars on top of each other, starting with the tallest (whole category) bar, or by drawing the subbars as differences between their respective counts. The result will be visually identical.

In contrast, the mean of a subset can be greater than the mean of the superset, and so the subbars representing the selection may end up taller then the bar representing all cases within the category. As a result, if we were to layer the bars on top of each other, we could end up with overplotting.

### Combining subsets {#associativity}

Finally, as displayed in Figure \@ref(fig:stacking), when multiple selections/groups are present, can we combine the summaries in a meaningful way? This is the essence of stacking. For counts, there is the simple idiomatic way: simply sum up the counts within the category. The resulting quantity will be the same as if we had ignored the grouping variable.   

For means, there is no such meaningful way to combine the statistics. If we were to apply the same operation as with counts and sum up the group means, the resulting statistic would not be meaningful. We could also try to take the mean of the group means, however, the resulting statistic would be different from the grand mean within the category. 
...
Means do not share these properties: mean of an empty selection is not defined, the mean of a selection is not guaranteed to be less than the mean of the whole, and the mean of group means will most likely be different from the grand mean. Importantly, these properties or lack thereof are not tied to any specific plot type (e.g. a barplot) but are instead tied to underlying statistic.

One might wonder if these problems are just a quirk of stacking. However, we run into some of the same issues with any other interactive feature that deals with summary statistics computed on parts of the data. If, for example, on top of linked brushing, we wanted to implement a pop-up window that displays summary statistics within a given object as text, we would still have to contend with the problem of how to display empty selections. Here, there is perhaps a simple solution: we could display the mean of an empty selection as some special value, for example `NA`, `undefined`, or an empty string, `""`. However, by doing so, we have to step outside of the value type of the summaries on non-empty selections (real number/`float`). While this may not be a major problem in most types of statistical software, it still does add some complexity to our code, since whatever functions use the summaries (such as the display function) now have to be generic over `float` and `NA`/`underfined`/`""`. On the other hand, there is something natural about counts having the value for empty selections (zero) be of the same type as all other values (`int`).

Claus Wilke summarized the problem as follows: "Stacking is useful when the sum of the amounts represented by the individual stacked bars is in itself a meaningful amount" [@wilke2019, p. 52]. This is a very concise and effective way to communicate the main point. However, it also misses something subtle yet important. Is "summing" the only way to implement stacking? The short answer is "no". For example, products work just as well as sums for linked brushing of stacked objects, see Figure \ref{fig:sumsproducts}. In fact, there is whole a class of mathematical functions, or more precisely mathematical structures, that share the properties listed above. To discuss these, let's first lay the groundwork with some theory.   

```{r sumsproducts}
#| echo: false
#| fig-height: 3
#| fig-cap: "Sums and products work just as well as counts when used in an interactive barplot. Specifically, they provide a way of displaying empty selections, a stable visual reference, and the ability to combine subsets in a meaningful way. Note the different y-axis limits. a) The underlying data. b) Barplot of products. c) Barplot of sums"
...
With these ideas in place, we can try to outline something we provisionally call "fluent interactive graphics". It starts from several axioms:

1. Linked brushing is useful
2. Ergo, every object in every plot should be selectable and able to represent selections
3. This should be congruous with other interactions (e.g. changing histogram binwidth/anchor)
