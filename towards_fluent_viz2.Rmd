...
Are counts unique in being so well-behaved in this way? The short answer is "no". For example, sums or products (of values $\geq 1$) conform to all three properties listed above. In other words, in the context of linked brushing, a stacked barplot will behave equally well if it display sums (not surprising) or products (perhaps somewhat surprising?), see Figure \ref{fig:sumsproducts}. In fact, there is whole a class of mathematical functions, or more precisely mathematical structures, that share these nice properties. To discuss these, let's first lay the groundwork with some relevant theory.   

```{r sumsproducts}
#| echo: false
#| fig-height: 3
#| fig-cap: "Sums and products work equally well as counts when it comes to displaying empty selections, providing a stable visual reference, and being able to be combined together. Note the different y-axis limits. a) The underlying data. b) Barplot of products. c) Barplot of sums"
...
We propose that any summary computed on the parts should form a commutative monoidal preorder. To rephrase Section \ref{monoids}, the summaries should consists of a set $M$ with elements that can be at least partially ordered (such that, for any $m_1$ and $m_2$, we may determine whether $m_1 \leq m_2$), a neutral element $e$, and a binary operation $\otimes$ such that:
...
Why might these properties be desirable for summaries represented by interactive visualizations? Let's go through them one by one. Property 1. says that there is a valid way of representing "nothing" in our summary. As we have seen in \ref{empty}, this is useful since, for example, the absence of an object unambiguously indicates an empty selection. Property 2. says that combining   

Requiring summaries to be commutative monoidal preorder has another benefit. This condition means that we can always compute the summaries in a single pass through the data, via a *reduce*/*fold*/*accumulate* function [see e.g. @abelson2022, p. 53]. Specifically, we can reduce the data into $k$ collections of summaries on the $k$ parts (within one partition level) by first initializing each collection with the default values (monoidal units), and then iterating through the $n$ data points and updating $j$th collection on each step, with $j$ being the factor level corresponding to the $i$th datapoint. 
